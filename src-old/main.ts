...
function getFramebufferStatusString(gl: WebGL2RenderingContext, status: number): string {
    switch(status) {
        ...
        case gl.FRAMEBUFFER_UNSUPPORTED:
            return "FRAMEBUFFER_UNSUPPORTED (float textures may not be supported!)";
...
function Render2Texture(renderer:OpenGLRenderer, gl_context:WebGL2RenderingContext,camera:Camera,shader:ShaderProgram,cur_texture:WebGLTexture){
    ...
    if (status !== gl_context.FRAMEBUFFER_COMPLETE) {
        console.error("Framebuffer not complete in Render2Texture: " + getFramebufferStatusString(gl_context, status));
        console.error("This usually means float texture rendering is not supported on your system!");
    }
...
function main() {
  ...
  const floatLinearExt = gl_context.getExtension('OES_texture_float_linear');
  if(!floatLinearExt){
        console.warn("WARNING: OES_texture_float_linear not supported - filtering may be limited");
    }
    
  const floatTextureExt = gl_context.getExtension('OES_texture_float');
  if(!floatTextureExt){
      console.error("CRITICAL: OES_texture_float not supported - simulation will not work!");
      alert("Your browser/GPU does not support float textures. The simulation will not work properly.");
  }
    
  const colorBufferFloatExt = gl_context.getExtension('EXT_color_buffer_float');
  if(!colorBufferFloatExt) {
      console.error("CRITICAL: EXT_color_buffer_float not supported - simulation will not work!");
      alert("Your browser/GPU does not support rendering to float textures (EXT_color_buffer_float). This is a known limitation on macOS. The erosion simulation will not function properly.\n\nRecommended: Use Chrome or Firefox on Windows/Linux for full support.");
  } else {
