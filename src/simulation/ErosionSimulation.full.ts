...
export class ErosionSimulation {
  ...
  constructor(gpuContext: GPUContext, layerCompute: LayerCompute) {
    console.log("🔥 FULL ErosionSimulation constructor starting - WITH COMPUTE PIPELINES! 🔥");
  ...
  async initializeTerrain(layerStack?: any) {
    ...
    console.log("ErosionSimulation height texture format:", this.heightTexture.format);
    
    if (computedTexture.format !== this.heightTexture.format) {
      console.error("TEXTURE FORMAT MISMATCH! Cannot copy textures with different formats.");
      console.error("Source:", computedTexture.format, "Destination:", this.heightTexture.format);
      return;
    }
  ...
  addRainAtPosition(normalizedX: number, normalizedY: number) {
    ...
    console.log(`Adding rain at: (${normalizedX.toFixed(3)}, ${normalizedY.toFixed(3)}) -> texture (${this.mouseRainPosition.x}, ${this.mouseRainPosition.y})`);
