...
export class LayerCompute {
    ...
    constructor(gpuContext: GPUContext) {
        ...
        // Create output texture (RGBA8Unorm to support linear filtering)
    ...
    public async computeLayers(layerStack: LayerStack): Promise<void> {
        ...
        this.gpuContext.device.queue.writeBuffer(this.layerBuffer, 0, layerData.buffer);
    ...
    public uploadImageToArray(imageData: ImageData, arrayIndex: number): void {
        if (arrayIndex >= this.maxImageLayers) {
            throw new Error(`Image array index ${arrayIndex} exceeds maximum ${this.maxImageLayers}`);
        }
        ...
        const resizedImageData = ctx.getImageData(0, 0, this.textureSize, this.textureSize);
